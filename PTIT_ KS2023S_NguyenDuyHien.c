...
void timKiemSinhVien() {
    char tenCanTim[50];
    int timThay = 0;
    int low = 0, high = soLuongSinhVien - 1;
    ...
    while (low <= high) {
        int mid = (low + high) / 2;
        if (strcmp(studentList[mid].name, tenCanTim) == 0) {
            inDanhSachSinhVien(); // In ra thông tin cua sinh viên tim thay
            timThay = 1;
            break;
        } else if (strcmp(studentList[mid].name, tenCanTim) < 0) {
            low = mid + 1;
        } else {
            high = mid - 1;
        }
